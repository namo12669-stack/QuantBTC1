...
@pytest.mark.parametrize('family',['trend_pullback','breakout','divergence','pair_spread','lead_lag'])
...
def test_signal_engine_does_not_bridge_missing_bar(cfg):
    ...
    out=generate_signals(b,p,Candidate('divergence','ETHUSDT'),cfg)
    assert all(s.time != gap for s in out)


def test_sparse_clock_must_not_collapse_gap(cfg):
    b=bars(400);b=b.drop(b.index[100])
    with pytest.raises(DataError,match='REGULAR_HOURLY_GRID'):
        generate_signals(b,None,Candidate('breakout',None),cfg)


def test_wilder_seed_is_sma():
    from btc_quant.signals import rma
